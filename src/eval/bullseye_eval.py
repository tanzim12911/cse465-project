...
def run_bullseye_evaluation(model, processor, dataset_split, output_dir, num_samples=None):
    dispatcher = BullsEyeDispatcher(taxonomy_path="./src/router/taxonomy_map.json")

    os.makedirs(output_dir, exist_ok=True)
    output_file = os.path.join(output_dir, "bullseye_results.jsonl")
    ...
    for idx, item in enumerate(tqdm(samples, desc="BullsEye Evaluating")):
        ...
        if original_image is not None and hasattr(original_image, 'size'):
            max_side = 672
            w, h = original_image.size
            if max(w, h) > max_side:
                scale = max_side / max(w, h)
                original_image = original_image.resize((int(w*scale), int(h*scale)))
        ...
        if "Final Answer:" in output_text:
            ...
            prediction = after[0].upper() if after else "N/A"
        else:
    ...
    print(f"\nOverall BullsEye Accuracy: {overall:.2f}%  ({correct}/{total})")
    ...
    summary_file = os.path.join(output_dir, "bullseye_summary.json")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("Dataset not found locally. Please run download_colorbench.py first.")
        exit(1)

    model, processor = load_model_and_processor(args.model_id)
    run_bullseye_evaluation(model, processor, eval_data, args.output_dir, args.num_samples)
